...
public class NfcPlugin extends CordovaPlugin implements NfcAdapter.OnNdefPushCompleteCallback {
    ...
    private static final String READ_TAGMF = "readTagMF";
    private static final String WRITE_TAGMF = "writeTagMF";
    ...
    private static final String READ_MF = "readMF";
    ...
    public boolean execute(String action, JSONArray data, CallbackContext callbackContext) throws JSONException {
        ...
        } else if (action.equalsIgnoreCase(READ_TAGMF)) {
            readTagMF(data, callbackContext);

        } else if (action.equalsIgnoreCase(WRITE_TAGMF)) {
            writeTagMF(data, callbackContext);
    ...
    private void readTagMF(JSONArray data, CallbackContext callbackContext){
      if (getIntent() == null) {  // TODO remove this and handle LostTag
        callbackContext.error("Failed to read tag, received null intent");
      }
        JSONObject json  = MiFare.readNFC(data, savedIntent);

      String command = MessageFormat.format(javaScriptEventTemplate, READ_MF, json);
      Log.v(TAG, command);
      this.webView.sendJavascript(command);
      callbackContext.success();
    }

    private void writeTagMF(JSONArray data, CallbackContext callbackContext){
      if (getIntent() == null) {  // TODO remove this and handle LostTag
        callbackContext.error("Failed to write tag, received null intent");
      }

        boolean wrote = MiFare.writeNFC(data, savedIntent);

      if(wrote){
        callbackContext.success();
      }else{
        callbackContext.error("Failed to write tag, log in fail");
      }
    }
