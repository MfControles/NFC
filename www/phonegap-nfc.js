...
var nfc = {
    ...
    readMF: function (jsonMessage, callback, win, fail) {
        console.log('readMF: function', jsonMessage);
        // La funcion queda a la espera del evento "readMF", que ejecutará java una vez haya
        // leído la data desde la tarjeta
        if(listenerValidator===null){
          document.addEventListener("readMF", callback, false);
          listenerValidator=callback;
        }
    
        cordova.exec(win, fail, "NfcPlugin", "readTagMF", [jsonMessage]);
      },
    
    writeMF: function (jsonMessage, win, fail) {
        console.log('writeMF: function', jsonMessage);
        cordova.exec(win, fail, "NfcPlugin", "writeTagMF", [jsonMessage]);
    },
